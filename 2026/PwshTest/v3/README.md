...
起動: `wincli.ps1 [-TargetPid <pid>] [-TargetHandle <hwnd>] [-OutputDir <path>] [-h]`

標準入力からコマンド受信し、指定したプロセスまたはウィンドウハンドルに対し処理を実行します。
ターゲットを指定しない場合、新しい `pwsh` プロセスを起動して制御対象とします。
出力（スクリーンショットやテキスト）は `-OutputDir` で指定したディレクトリ、またはカレントディレクトリの `.wincli` フォルダに保存されます。
...
- `k keys`: キー押下 (.NET SendKeys 準拠)
  - ※キー送信時は対象を前面に移動します。
- `dump`: テキストコンテンツの取得 (UI Automation TextPattern)
  - ファイル名は `.\.wincli\dump.<ID>.<yymmdd.hhmmss>.txt`
  - ※対象のアプリケーションが UI Automation の TextPattern をサポートしている必要があります（Windows Terminal, Notepad 等）。
- `exit`: 終了
