...
[DllImport("user32")]public static extern bool ScreenToClient(IntPtr h,ref POINT p);
    ...
    [DllImport("user32")]public static extern void SetProcessDPIAware();[DllImport("user32")]public static extern void SetProcessDpiAwarenessContext(IntPtr c);
...
function Get-Hwnd($t){
  if(!$t -or $t -eq '$null'){return 0}
  $p=ps|? MainWindowTitle -like "*$t*"|select -f 1
  if(!$p){$p=ps|? MainWindowTitle -match $t|select -f 1}
  if($p){$p.MainWindowHandle}else{0}
}
function Get-ScreenRect{
  $s=[System.Windows.Forms.Screen]::PrimaryScreen.Bounds
  $r=New-Object RECT;$r.Right=$s.Width;$r.Bottom=$s.Height
  return $r
}
...
function Get-Target($h,$x,$y){
    $p=New-Object POINT;$p.x=$x;$p.y=$y
    $t=[Win32]::WindowFromPoint($p)
    if($t -eq 0){$t=$h}
    [void][Win32]::ScreenToClient($t,[ref]$p)
    @{h=$t;l=Make-LParam $p.x $p.y}
}

function Get-TargetGlobal($x,$y){
    $p=New-Object POINT;$p.x=$x;$p.y=$y
    $t=[Win32]::WindowFromPoint($p)
    if($t -ne 0){[void][Win32]::ScreenToClient($t,[ref]$p)}
    @{h=$t;l=Make-LParam $p.x $p.y}
}
