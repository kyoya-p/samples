...
class WebRTCController(val signalingUrl: String) {
    ...
    fun connectP2P() {
        scope.launch {
            if (iceServers == null) {
                println("ICE servers not ready, fetching...")
                fetchIceServers()
            }
            
            if (iceServers == null) {
                println("Error: Failed to obtain ICE servers. Cannot start P2P.")
                return@launch
            }
            ...
            scope.launch {
                when (data.type) {
                    "offer" -> {
                        ...
                        if (iceServers == null) {
                            println("ICE servers not ready for offer, fetching...")
                            fetchIceServers()
                        }
                        if (iceServers == null) {
                            println("Error: Cannot handle offer without ICE servers.")
                            return@launch
                        }
