/**
 * Browser-side Test Client for WebRTCSDK
 */
...
const sdk = new WebRTCSDK(signalingUrl, {
    iceServers: [
        ...
        {
            urls: [
                `turn:${location.hostname === 'server' ? 'turn' : location.hostname}:3478?transport=udp`,
                `turn:${location.hostname === 'server' ? 'turn' : location.hostname}:3478?transport=tcp`
            ],
            username: 'user',
...
sdk.onMessage = (from, data) => appendLog(`[MSG from ${from}] ${data}`);
sdk.onLog = (msg) => appendLog(`[SDK] ${msg}`);
