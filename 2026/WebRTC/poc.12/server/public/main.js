...
window.addEventListener('load', () => {
    ...
    noHostCheck.addEventListener('change', () => {
        ...
        if (pc && pc.localDescription && sdpArea) sdpArea.value = getProcessedSDP();
    });
...
function updateSDPArea() {
    if (pc && pc.localDescription && sdpArea) {
...
if (createOfferBtn) createOfferBtn.onclick = initiateCall;

const createAnswerBtn = document.getElementById('createAnswerBtn');
if (createAnswerBtn) {
    createAnswerBtn.onclick = async () => {
        if (!sdpArea || !sdpArea.value) return alert("Please paste Offer SDP first");
        setupPC();
        try {
