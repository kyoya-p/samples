...
- **Windows (MSVC) 対応**:
    ...
    - **システムリンクター**: Linux では `pkg-config` で依存関係を解決するが、Windows では `CMakeLists.txt` で `bcrypt`, `ws2_32` などのシステム .lib を明示的に指定する。

## ログ・デバッグ
- **ファイル出力**: `app.log` 形式のファイルに実行ログを記録。
- **例外処理**: 致命的なエラー発生時はスタックトレース（Linux: `backtrace`, Windows: プレースホルダー）をログに記録し、安全に終了またはエラーを通知する。
