...
- **API Key 管理**:
    ...
    - 接続確立成功時、有効な API Key を `app.conf` に自動保存する。
- **設定ダイアログ ([Activate])**:
    - メイン画面またはアドレス帳画面から呼び出し可能なモーダルダイアログ。
    - **API Key 入力欄**: 現在のキーが表示され、編集可能。
    - **[Connect] ボタン**: 入力されたキーでFirebaseへ再接続を試み、成功時に設定を保存して閉じる。
    - **[Cancel] ボタン**: 変更を破棄して閉じる。
    - **エラー表示**: 接続失敗時、ダイアログ内に赤字でエラーメッセージを表示。
