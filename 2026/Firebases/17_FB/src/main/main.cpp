...
int main(int argc, char** argv) {
  try {
      ...
      static bool p_sort_desc = false;
      
      auto p_input_name = Input(&p_filter_name, "Filter");
      auto p_input_email = Input(&p_filter_email, "Filter");
      ...
      auto update_picker_list = [&, service_ptr=&service]() {
          ...
          auto it = std::remove_if(contacts.begin(), contacts.end(), [&](const Contact& c){
              std::string fn = p_filter_name; 
              std::string fe = p_filter_email;
              // Case insensitive simple check? Or just exact/partial. Let's do partial case-sensitive for simplicity or simple tolower.
              // Keeping it case-sensitive for now as per std::string::find default.
              if (!fn.empty() && c.name.find(fn) == std::string::npos) return true;
              if (!fe.empty() && c.email.find(fe) == std::string::npos) return true;
              return false;
          });
      ...
      auto p_btn_time = Button("Time", [&]{ 
          ...
          else {p_sort_col=2; p_sort_desc=false; p_filter_name=""; p_filter_email="";} 
          update_picker_list(); 
      }, ButtonOption::Ascii());
      ...
      // Header Layout
      auto p_header = Container::Horizontal({
          p_btn_name, p_input_name,
          p_btn_mail, p_input_email,
          p_btn_time
      });
      ...
      // We need to reconstruct p_header with the wrapped inputs
       p_header = Container::Horizontal({
          p_btn_name, p_input_name_c,
          p_btn_mail, p_input_email_c,
          p_btn_time
      });
       p_main_container = Container::Vertical({
          p_header,
          p_list_container,
          p_cancel_btn
      });
      ...
      auto picker_renderer = Renderer(p_main_container, [=, &service] {
          ...
          return vbox({
              ...
              hbox({
                  ...
                  (p_sort_col == 0 ? p_input_name->Render() : text("          ")) | size(WIDTH, EQUAL, 10),
                  text("  "),
                  p_btn_mail->Render(), sort_indicator(1), 
                  (p_sort_col == 1 ? p_input_email->Render() : text("          ")) | size(WIDTH, EQUAL, 10),
                  filler(),
          ...
          }) | border | center | bgcolor(Color::Blue) | size(WIDTH, GREATER_THAN, 80);
      });
      ...
      auto main_activate_btn = Button("[Activate]", [=] { *show_config = true; }, ButtonOption::Ascii());
      ...
      auto config_connect_btn = Button("[Connect]", [=, &service] { 
          if (service.Initialize(api_key_input_val, 20)) { 
              *show_config = false;
              // Save to app.conf
