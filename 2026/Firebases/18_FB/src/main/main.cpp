  ...
  try {
      ...
      auto refresh_address_list = [=, &service](const std::vector<Contact>& contacts) {
        ...
        for (size_t i = 0; i < contacts.size(); ++i) {
          ...
          auto row = Renderer(remove_btn, [=, &service, &contacts] {
            ...
            // Trigger LoadMore if this row is focused and near the end of the current list
            if (remove_btn->Focused() && idx >= (int)contacts.size() - 2) {
                service.LoadMore(10);
            }
      ...
      auto picker_renderer = Renderer(p_main_container_gen, [=, &service] {
          ...
          return vbox({
              ...
              separator(),
              hbox({
                  p_btn_name->Render(), sort_indicator(0), render_input(0, p_input_name), text("  "),
                  p_btn_mail->Render(), sort_indicator(1), render_input(1, p_input_email), text("  "),
                  filler(),
      ...
      std::function<void()> refresh_scan_ui = [&]() {
          ...
          for (const auto& email : scan_confirmed_emails) {
              auto remove_btn = Button("[Remove]", [=, &service, &screen] {
      ...
      auto main_menu_renderer = Renderer(menu_container, [&] {
          auto make_box = [&](Component c) {
              auto el = c->Render() | center | size(WIDTH, EQUAL, 31) | size(HEIGHT, EQUAL, 3);
              if (c->Focused()) el = el | inverted;
              return window(text(""), el);
          };
          return vbox({
