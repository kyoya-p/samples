...
### フェーズ 10: メモリ最適化と実行安定性の強化
- **大規模データ表示の最適化**:
    - `refresh_ui` 内のコンポーネント生成ループにおいて、各行のラムダ式が `contacts` ベクタ全体を値キャプチャしていた問題を修正。
    - 必要なフィールド（ID、名前、メール等）のみを個別に値キャプチャすることで、UI更新時のメモリ使用量と文字列表現のオーバーヘッドを大幅に削減。これにより Windows 環境で発生していた `bad_allocation` および `string too long` 例外を解消。
- **レンダリングの効率化**:
    - 各行の `Renderer` 内で行われていた ID による線形探索（インデックス特定）を廃止。ループ時にインデックスを確定させてキャプチャすることで、描画処理を $O(1)$ に高速化。
- **Windows固有のコンパイル競合の解決**:
    - `windows.h` の `min`/`max` マクロと `std::min` の競合を、関数呼び出しを括弧で囲む手法 (`(std::min)`) により解決。
- **堅牢な例外処理の導入**:
    - 予期せぬクラッシュ時に原因を特定できるよう、`main` 関数に詳細な例外キャッチと標準エラー出力へのメッセージ表示を一時的に実装し、デバッグ効率を向上。

### フェーズ 11: プロジェクト構造の整理と自動テストの導入
- **ディレクトリ構成の刷新**: 
    - プロジェクトの保守性向上のため、ソースコードを `src/main`、テストコードを `src/test` に分離。
    - プロダクトコード（`main.cpp`, `dataaccess.*`, `utils.*`）を `src/main` へ移動し、`CMakeLists.txt` のビルド設定を追従。
- **自動テスト基盤の構築 (Google Test)**:
    - **Google Test (GTest)** を統合し、ヘッドレス環境でのUIおよびロジック検証を可能にした。
    - `src/test/app_test.cpp` を作成し、FTXUIのイベントシミュレーション (`OnEvent`) を用いたUIテスト、`Screen` を用いたレンダリング検証、およびサービス層の初期状態テストを実装。
- **設計ドキュメントの同期**:
    - `SoftwareDesign.md` を更新し、新構成およびテスト設計の方針（イベントシミュレーション、スナップショットテスト等）を明文化。
