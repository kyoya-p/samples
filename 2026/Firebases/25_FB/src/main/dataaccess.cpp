...
void FirestoreService::FetchNextPage() {
...

    {
        ...
        is_loading_ = true;
        page_index = is_resetting_ ? 0 : pages_.size();
        if (!is_resetting_) {
...

    {
        ...
        if (page_index < pages_.size()) {
            ...
            // New listener for page 0 will be stored in callback
        } else {
            registration.Remove();
        }
    }
...
void FirestoreService::ClearCollection(const std::string& collection_name) {
    if (!firestore_) return;
    firestore_->Collection(collection_name.c_str()).Get().OnCompletion([this, collection_name](const firebase::Future<firebase::firestore::QuerySnapshot>& future) {
        if (future.error() == firebase::firestore::Error::kErrorOk) {
            ...
            if (collection_name_ == collection_name) StartQuery();
        }
    });
}
