...
int main(int argc, char** argv) {
  try {
      ...
      auto update_picker_list = [&]() {
          ...
          for (size_t i = 0; i < total; ++i) {
              ...
              auto btn = Button("", [&, idx] {
                  std::string email = addr_service.GetData(idx, "email");
                  if(!email.empty()) {
                      if (std::find(scan_confirmed_emails.begin(), scan_confirmed_emails.end(), email) == scan_confirmed_emails.end()) {
          ...
          if (addr_service.IsLoading()) {
              p_list_container->Add(Renderer([&] { return text("Loading...") | dim | center; }));
          }
      };
      ...
      auto picker_renderer = Renderer(p_main_container, [&] {
          ...
          }) | border | bgcolor(Color::Blue) | size(WIDTH, GREATER_THAN, 90) | clear_under;
      });
      ...
      auto new_enter_btn = Button("[Enter]", [&] { 
          if (!scan_new_email.empty()) { 
              if (std::find(scan_confirmed_emails.begin(), scan_confirmed_emails.end(), scan_new_email) == scan_confirmed_emails.end()) {
      ...
      auto refresh_scan_ui = [&]() {
          ...
          for (const auto& email : scan_confirmed_emails) {
              ...
              scan_list_container->Add(Renderer(remove_btn, [email, remove_btn] { 
                  return hbox({ text("- " + email) | flex, remove_btn->Render() }); 
              }));
          }
      };
      ...
      auto send_btn = Button("[Send]", [&] { 
          ...
          log_service.AddJobLog(scan_confirmed_emails);
      ...
      auto scan_renderer = Renderer(Container::Vertical({ new_email_input_c, new_enter_btn, new_addr_btn, scan_list_container_c, send_btn, close_app_btn }), [&] {
          return vbox({
              text("Send") | bold,
              separator(),
              text("To:"),
              hbox({
                  text("- "),
                  new_email_input->Render() | border | size(WIDTH, EQUAL, 40),
                  text("                "),
              ...
              separator(),
              hbox({ filler(), send_btn->Render(), text("  "), close_app_btn->Render() })
          }) | border;
      });
