...
- **プロセス構造の解明**:
    - Windows Terminal (`WindowsTerminal.exe`) 環境下では、`pwsh.exe` は独自のウィンドウハンドル (`MainWindowHandle`) を持たず、親である Terminal プロセスが描画を管理していることを特定。
- **外部入力の制御（SendKeys）**:
    - `pwsh` プロセスに対して `SendKeys` を送ることは不可能だが、親である `WindowsTerminal.exe` に対してウィンドウタイトル等で特定して送信することで、内部のシェルを操作可能であることを実証。
- **TUIアプリの入力特性**:
    - FTXUI 等の TUI アプリは標準的な Win32 メッセージ（`WM_CHAR` 等）をバイパスし、コンソール入力バッファを直接参照しているため、`PostMessage` による制御が効かない特性を確認。
- **MSVCビルドの安定化**:
    - `NOMINMAX` の定義やラムダキャプチャの明示化により、MSVC 固有のコンパイルエラー（C3489 等）を解消。

### フェーズ 25: UI/UXの洗練と診断機能の強化
- **視覚的ノイズの削減**:
    - `name`, `time`, `op` カラムの各要素において、フォーカス時およびマウスホバー時の反転表示（ハイライト）を廃止。
    - 操作ボタン（Add, Close, Remove）のホバー装飾も排除し、シンプルで一貫性のある外観に変更。
- **選択状態の可視化改善**:
    - 行選択（アドレスクリック）時のインジケータ `>` を、Emailボタン内から行の先頭（Nameカラム）に移動。
- **メンテナンス性の向上**:
    - `src/main/main.cpp` のすべての関数に対して、役割を簡潔に示す1行コメントを付与。
- **外部診断ツールの開発**:
    - Windows API を利用して Windows Terminal ウィンドウを特定し、その表示領域を直接 PNG 保存するスクリプト (`capture_terminal.ps1`) を作成。これによりアプリ外からのエビデンス採取を容易にした。
