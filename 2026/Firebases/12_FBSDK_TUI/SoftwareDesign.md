...
# 5.5. 自動テスト
- **テストフレームワーク**: Google Test (GTest) を使用。
- **UIテスト戦略**:
    - **イベントシミュレーション**: FTXUIの `OnEvent` メソッドに `Event::Return` や `Event::Character` を直接渡すことで、ユーザーのキー入力やボタン操作をヘッドレス環境で再現・検証する。
    - **レンダリング検証**: `ftxui::Screen` を使用してUIをメモリ上に描画し、出力文字列 (`ToString()`) をスキャンすることで、特定の情報が正しく表示されているかを検証する。
- **テストの分離**: プロダクトコード (`src/main`) とテストコード (`src/test`) を物理的に分離し、テストバイナリ (`app_tests`) が製品版バイナリのサイズやパフォーマンスに影響を与えない構成とする。
