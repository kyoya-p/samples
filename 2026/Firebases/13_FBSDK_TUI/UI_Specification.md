...
# 1. 概要
本アプリケーションは、オンラインアドレス帳アプリケーションである。Cloudサーバとリアルタイムに通信し、データの閲覧、追加、削除を行うことができる。
...
- **[Activate] 設定ダイアログ**:
    ...
    - **API Key 入力欄**: 現在のAPI-Keyが表示され、編集可能。
    - **[Connect] ボタン**: 入力されたキーを使用してクラウド接続を再初期化し、メイン画面へ戻る。
...
- **表示項目**:
    ...
    3. **Created At**: 作成日時 `YY/MM/DD HH:MM` 形式（固定幅 16文字）
    4. **Operation**: 操作ボタン（固定幅 10文字）
- **ソート順**:
  - カラムタイトルクリックでそのフィールドについて降順→昇順→降順を繰り返す(Operationカラム除く)
  - 昇順'▲'、降順'▼'、の文字をカラム名に付与
  - デフォルト: `timestamp` フィールドの降順。
...
## 3.3 データ操作
- **選択**:
  - アドレスリストをクリックした場合、クリックした対象のname,mailを[Add]フィールドにコピーし編集可能にする。
- **新規追加**:
  - フッターの入力エリアを使用。
  - **Name** および **Email** 欄にはランダムな初期値が自動入力される(テスト目的)
  - ユーザーは値を編集可能。
  - `[Add]` ボタン押下で Firestore にドキュメントを作成（IDはNameを使用）。
  - 追加成功後、入力欄には新たなランダム値がセットされる。
- **削除**:
  - リスト各行の `[Remove]` ボタン押下で、該当ドキュメントを即時削除。
  - アイテム選択中 'Delete'キー押下で、該当ドキュメントを即時削除。
...
- **操作方法**:
    ...
    - **キーボード操作**:
        ...
        - **Enter**: 
            - メイン画面: フォーカスに関わらず新規データの `[Add]` を実行。
            - 設定画面: 入力されたキーで `[Connect]` を実行。
            - フォーカスされたボタンの実行。
        - **a**: 
            - メイン画面で設定画面（Activate）を開く。
            - ボタン表示は `[<u>A</u>ctivate]` となり、ショートカットを明示。
        - **Esc**:
            - 設定画面を閉じる（Cancel）。
            - メイン画面でアプリケーションを終了（Close）。
        - **q**: アプリケーションの終了（ショートカット）。
        - **Delete**: 選択中の行（リスト項目）を削除。
        - **文字入力**: フォーカスされた `Name` / `Email` / `API Key` フィールドへのテキスト入力。
