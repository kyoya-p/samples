...
### フェーズ 11: プロジェクト構成の再整備とビルド修正
- **Windowsマクロ競合の修正**:
    - `SoftwareDesign.md` の記述に基づき、`main.cpp` において `windows.h` のインクルード後に `#undef RGB` を追加。これにより、FTXUI の `Color::RGB` との潜在的なシンボル競合を未然に防止。
- **ソースディレクトリ構成の変更**:
    - `Readme.md` で定義された構成に合わせ、すべてのソースコード (`.cpp`, `.hpp`) をプロジェクトルートから `src/` ディレクトリに移動。
    - `CMakeLists.txt` の `add_executable` を更新し、新しいソースパスを正しく反映。
- **ビルドディレクトリの標準化**:
    - `Readme.md` の記述に従い、Linux環境におけるビルド成果物の出力先を `build/` から `build/linux/` に変更。CI/CDやマルチプラットフォームでのビルド管理を容易にした。
