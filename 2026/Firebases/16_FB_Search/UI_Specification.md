...
- **[Activate] 設定ダイアログ**:
    ...
    - **[Connect] ボタン**:  変更を保存してメイン画面へ戻る。入力されたキーを使用して Firebase を再初期化する。
...
- **ソート順**: 
  - カラムタイトルクリックでソートオーダを指定。昇順/降順トグル。 
  - default: `timestamp` フィールドの昇順（古い順）。
- **フィルター/検索**
  - カラムタイトルに検索文字指定フィールド追加。
  - Name,Addressについて、文字列入力フィールドを持ち文字列先頭一致でフィルタ可能
