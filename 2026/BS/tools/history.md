...
### 2. Playwrightによるサイト構造의 再調査とツール開発
...
- **ショートコードの実装**: 
    ...
    - 日本語入力との併用も可能。
- **ヘルプの日本語化**: CLI의 ヘルプメッセージ（オプション説明、引数、コマンド概要）をすべて日本語化。
...
- 以下の主要セットおよびXレアを含む大量のカードを Neo4j にインポート。
    ...
    - **BS10 (戦竜)**: ブレイヴ（Brave）関連のカードを多数登録。`幻羅星龍ガイ・アスラ`（超覚醒）、`月光龍ストライク・ジークヴルム` 等を登録。
    - **BS11 (覇鳥)**: `太陽神龍ライジング・アポロドラゴン`、`滅神星龍ダークヴルム・ノヴァ`（合体破壊/BP+10k）等のキーカードを登録。
    - **BS12 (月の武神)**: 12宮Xレア時代への移行期カードを処理。

### 3. データインポートの効率化
- バッチ処理（100〜150件単位）による安定したデータ登録を実現。
- Cypher クエリの最適化により、大量のプロパティとリレーションを一括で `MERGE` するロジックを確立。
- 複雑なネスト構造を持つパラメータのデバッグを行い、フラットなパラメータ渡しによる安定動作を確認。

## 成果物更新
- **Neo4j Database**: BS01からBS12（一部）までの主要カードデータがグラフ化され、検索可能な状態。
- **登録済みカードID範囲**: `BS01-001` 〜 `BS12-038` (進捗に応じたバッチ処理)

## 備考
- `BS09-X35 超神星龍ジークヴルム・ノヴァ` などの歴史的なキーカードの効果を精密にマッピング。
- `滅神星龍ダークヴルム・ノヴァ` (BS11-X02) や `ウィッグバインド` (BS11-082) 等の強力な制限カード情報も反映。

# 作業ログ: サブコマンド化と Cypher 生成自動化 (2026-01-31)

## 実施内容

### 1. CLI のサブコマンド構造化 (`Main.kt`)
- `Clikt` のサブコマンド機能を導入し、`search` と `generate-cypher` の2つの主要機能を分離。
- 各サブコマンドごとに独立したヘルプメッセージとオプション管理を実現。
- 実行形式を `bscard.exe <subcommand> [options]` へ変更。

### 2. Neo4j Cypher クエリ生成機能の実装 (`GenerateCypher.kt`, `CypherGenerator.kt`)
- キャッシュされた YAML/JSON データから Neo4j へのインポート用 Cypher クエリを自動生成するツールを開発。
- **データ抽出の高度化**:
    - 効果テキストからキーワード（【...】）および発揮タイミング（『...』）を正規表現で自動抽出し、独立したノードとして関連付け。
    - カテゴリ、属性（色）、系統、レアリティをそれぞれ正規表現とマッピングで正規化し、グラフ構造へ反映。
- **一括処理対応**: キャッシュディレクトリ内の全ファイルをスキャンし、一括で Cypher スクリプトを出力する機能を実装。

### 3. キャッシュ形式の統一とシリアライズの修正
- キャッシュファイルの拡張子を `.yaml` に再統一。
- `Main.kt` において、`Json` ではなく `Yaml.default.encodeToString` を使用して、型安全かつ人間が読みやすい形式での保存を確実に実行。

### 4. ドキュメントの刷新 (`Readme.md`)
- 新しいサブコマンド構造、追加されたオプション（`--id`, `-o` 等）、および正しいキャッシュ拡張子を反映。
- ユーザー向けの利用例をサブコマンド別に整理。

### 5. 大規模データ採取の継続 (BS09)
- 「BS09」キーワードによる検索を実行。
- 全90枚のカードを検出し、未取得分の詳細データを自動収集・キャッシュ化。
- パース結果の検証により、シンボル画像の `alt` 属性に起因するパース揺れ（`BS03-1211` 等のID混入）を特定。次フェーズでの修正課題として管理。

## 成果物更新
- `tools/shared/src/Main.kt`: サブコマンド対応版エントリポイント
- `tools/shared/src/GenerateCypher.kt`, `CypherGenerator.kt`: Cypher 生成エンジン
