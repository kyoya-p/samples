...
//fun List<Any>.toJsonArray(): JsonArray = buildJsonArray { forEach { v -> add(v.toJsonElement()) } }
fun Any.toJsonArray(): JsonArray = buildJsonArray {
    val t = this@toJsonArray
    when (t) {
        is JsonArray -> t
        is List<*> -> (t as List<Any>).forEach { v -> add(v.toJsonElement()) }
    }
}
