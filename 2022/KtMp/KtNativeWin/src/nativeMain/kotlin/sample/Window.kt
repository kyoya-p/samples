...
fun WndProc(hwnd: HWND?, msg: UINT, wParam: WPARAM, lParam: LPARAM): LRESULT {
    ...
    when (msg) {
        // This message type is used by the OS to close a sample.window. Just closes the sample.window using DestroyWindow(hwnd);
        ...
        // WM_DESTROY message is sent to the sample.window, which actually closes it.
        ...
        // When no message type is handled in your application, return the default sample.window procedure. In this case the message
...
fun window() {
    ...
    memScoped {
        ...
        // In order to be able to create a sample.window you need to have a sample.window class available. A sample.window class can be created for your
        // application by registering one. The following struct declaration and fill provides details for a new sample.window class.
        ...
        // This function actually registers the sample.window class. If the information specified in the 'wc' struct is correct,
        // the sample.window class should be created and no error is returned.
        if ((RegisterClassEx!!)(wc.ptr) == 0u.toUShort()) {
            println("Failed to register!")
            return
        }

        // This function creates the first sample.window. It uses the sample.window class registered in the first part, and takes a title,
        ...
        // Everything went right, show the sample.window including all controls.
        ShowWindow(hwnd, 1)
        UpdateWindow(hwnd)


        // This part is the "message loop". This loop ensures the application keeps running and makes the sample.window able to receive messages
        // in the sample.WndProc function. You must have this piece of code in your GUI application if you want it to run properly.
        val Msg = alloc<MSG>()
