...
fun main(): Unit = with(SystemFileSystem) {
    ...
    fun String.toRecord() = Json { ignoreUnknownKeys = true }.decodeFromString<R>(this)
    generateSequence { source(Path("C:\\Users\\kyoya\\Desktop\\works\\ag\\agent.log")).buffered().readLine() }
        .mapNotNull { Regex("StatusReport\\[(.*)]").find(it)?.groupValues?.getOrNull(1) }
        .mapNotNull { runCatching { it.toRecord() }.getOrNull() }
        .forEach {
            println(it)
        }
}

data class R(  val environmentalInfo:EnvironmentalInfo, val srdmUsageInfo:SrdmUsageInfo)
data class EnvironmentalInfo(val usageInfo:UsageInfo, val systemInfo:SystemInfo)
data class UsageInfo(val systemUsage:SystemUsage, val vmProcessUsage:VmProcessUsage, val heapUsage:HeapUsage, val threadUsage:ThreadUsage)
data class SystemUsage(val cpuUsage:Double, val cpuLoadAverage:Double, val physicalMemoryTotal:Long, val physicalMemoryFree:Long, val swapSpaceTotal:Long, val swapSpaceFree:Long, val diskSpaceTotal:Long, val diskSpaceFree:Long, val diskUsabeSpace:Long)
...
data class ThreadUsage(val threadCount:Int, val totalStartedThreadCount:Int, val peakThreadCount:Int, val daemonThreadCount:Int)
data class SystemInfo( val hwInfo:HwInfo)
data class HwInfo(val numberOfProcessor:Int, val physicalMemory:Long, val virtualMemory:Long, val swapSpace:Long, val diskSpace:Long)
data class SrdmUsageInfo(val devices:Int, val reportLog:Map<String, String>, val dbStorageSize:Int, val userNumber:Int, val accessCount:AccessCount, val accessBrowserCount:Map<String, String>, val storageFreeSpace:Int, val dbFileSize:Int)
