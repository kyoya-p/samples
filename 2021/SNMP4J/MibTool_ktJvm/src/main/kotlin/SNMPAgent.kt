...
val oidMap: Map<OID, Variable> = mapOf()
...
class SNMPAgent(
    val mib: TreeMap<OID, VariableBinding>,
    ...
    fun run2(): SNMPAgent {
        ...
        snmp.addCommandResponder { ev ->
            ...
            (0 until bulkRepeate).mapWithHandover(ev.pdu.variableBindings) {
                ...
                it.mapIndexed { i, vb ->
                    val resVB =
                        if ((ev.pdu.type == PDU.GETNEXT) or (ev.pdu.type == PDU.GETBULK)) mib.higherEntry(vb.oid)?.value else mib[vb.oid]
                    if (resVB != null) resVB
                    else if ((resVB == null) and (ev.pdu.type == PDU.GETBULK)) VariableBinding(vb.oid,
                        Null.endOfMibView)
                    else {
                        resPdu.errorStatus = PDU.noSuchName
                        resPdu.errorIndex = i + 1
