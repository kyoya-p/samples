...
import jp.`live-on`.shokkaa.scrambledIpV4AddressFlow
...
class Test_scrambledIpV4AddressFlow {
    ...
    fun test1_1() = runBlocking {
        ...
        val res = scrambledIpV4AddressFlow(adr, 8).toList()
    ...
    fun test1_2() = runBlocking {
        val adr = InetAddress.getByName("1.2.3.4")
        val res = scrambledIpV4AddressFlow(adr, 0).toList()
        assert(res.size == 1)
    ...
    fun test1_3() = runBlocking {
        val adr = InetAddress.getByName("1.2.3.4")
        val res = scrambledIpV4AddressFlow(adr, 1).toList()
        assert(res.size == 2)
        assert(res[0].hostAddress.apply(::println) == "1.2.3.4")
