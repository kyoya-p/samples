...
fun test11(deckOpt: Set<Card>, deckBase: List<Card>, seed: Int) {
    ...
    fun ParallelWorld.terminationCheck(): ParallelWorld {
        ...
        val depth = sumBy { it.world.ownSide.evaluation() } * 1.0 / count()
        if (nCond != 0) {
    ...
    val e = eden(deck1, enemyDeck = setOf())
    ...
    try {
        e.forEachIndexed { i, it ->
            //println("${seed}, ${deckOpt}, ${it.stn.step}, ${it.stn.ownSide}")
            print("$i ")
        }
    } catch (e: Exception) {
        e.printStackTrace()
